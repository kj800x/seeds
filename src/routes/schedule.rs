...
use crate::schedule::{
    PlantingTiming, StartMethod, generate_schedule_with_methods, parse_planting_timing_from_fields,
};
use crate::templates;
...
fn build_seeds_with_timing(
    planned: &[(Seed, Option<String>)],
) -> Vec<(Seed, PlantingTiming, Option<StartMethod>)> {
    planned
        .iter()
        .map(|(seed, method)| {
...
pub async fn schedule_page(State(state): State<AppState>) -> Result<Markup, AppError> {
    let current_year = chrono::Local::now().year();
    let planned_with_methods =
        queries::list_planned_seeds_with_method(&state.db, current_year as i64).await?;
...
pub async fn schedule_list(State(state): State<AppState>) -> Result<Markup, AppError> {
    let current_year = chrono::Local::now().year();
    let planned_with_methods =
        queries::list_planned_seeds_with_method(&state.db, current_year as i64).await?;
    let seeds_with_timing = build_seeds_with_timing(&planned_with_methods);
    ...
    let manual_seeds: Vec<&Seed> = all_seeds
        .iter()
...
pub async fn this_week(State(state): State<AppState>) -> Result<Markup, AppError> {
    ...
    let filtered: Vec<_> = all_actions
        .iter()
        .filter(|a| {
            (a.date >= today && a.date <= window_end) || (a.date < today && a.date >= start_of_week)
        })
        .cloned()
        .collect();

    let next_action = all_actions.iter().find(|a| a.date > window_end);
